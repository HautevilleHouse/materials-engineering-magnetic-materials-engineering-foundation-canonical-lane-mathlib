...
import HautevilleHouse.MaterialsEngineeringMagneticMaterialsEngineeringFoundationCanonicalLaneLean.MathlibObjects
...
def mathlibProofObligation : MathlibProofObligation := {
  sourceRepo := "magnetic-materials-canonical-lane",
  theoremName := "Magnetic Materials Foundation",
...
def commonCoreProjectionLawAvailable : Prop := forall {X : Type} [Add X] [Sub X] (L : AdditiveLane X), L.xNext = L.state + L.projection.toFun L.delta

def commonCoreCarriageLawAvailable : Prop := forall {X : Type} [Add X] [Sub X] (L : AdditiveLane X), L.carriedComponent = L.delta - L.projection.toFun L.delta

def commonCoreIdempotenceAvailable : Prop := forall {X : Type} [Add X] [Sub X] (L : AdditiveLane X), L.projection.toFun (L.projection.toFun L.delta) = L.projection.toFun L.delta
...
def theoremSpecificEndgamePilotClosed : Prop := forall A : AdmissibleClass, ConstrainedMagneticClosure A

theorem theorem_specific_endgame_pilot_checked : theoremSpecificEndgamePilotClosed := by
  intro A; exact constrained_magnetic_endgame A
